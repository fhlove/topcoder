    ...
    int MaxArea(vector <string> board)
    {
		...
		for(int i=n-1;i>=0;i--)
		{
			for(int j=m-1;j>=0;j--)
			{
				...
				int x=i+1;
				int y=j+1;
				if(x<n&&board[i][j]!=board[x][j])
				{
					h[i][j]+=h[x][j];
				}
				if(y<m&&board[i][j]!=board[i][y])
				{
					w[i][j]+=w[i][y];
				}
			}
		...
		for(int i=0;i<n;i++)
		{
			for(int j=0;j<m;j++)
			{
				...
				for(int k=0;k<hh;k++)
				{
					temp=min(temp,w[i+k][j]);
					res=max(res,temp*(k+1));
				}
			}
		}
		return res;
...
// BEGIN CUT HERE
	public:
	void run_test(int Case) { if ((Case == -1) || (Case == 1)) test_case_1(); if ((Case == -1) || (Case == 2)) test_case_2(); if ((Case == -1) || (Case == 3)) test_case_3(); if ((Case == -1) || (Case == 4)) test_case_4(); if ((Case == -1) || (Case == 5)) test_case_5(); if ((Case == -1) || (Case == 6)) test_case_6(); if ((Case == -1) || (Case == 7)) test_case_7(); }
	private:
